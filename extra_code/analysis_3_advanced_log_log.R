...
df_utilization <- df_clean %>%
  ...
  # Log transform all numeric predictors (add 1 to handle zeros)
  mutate(log_utilization_rate = log(utilization_rate)) %>%
  
  mutate(across(where(is.numeric) & !matches("log_utilization_rate"), ~ log(. + 1))) %>%
  na.omit() %>%
