...
cat("Utilization dataset (after removing NAs): ", nrow(df_utilization), "libraries\n")
cat("Mean utilization rate:", round(mean(df_utilization$utilization_rate), 3), "\n")
cat("Median utilization rate:", round(median(df_utilization$utilization_rate), 3), "\n\n")

# ============================================================
#   Train/Test Split (80/20)
# ============================================================
set.seed(123)
train_index <- createDataPartition(df_utilization$utilization_rate, p = 0.8, list = FALSE)
...
lm_model <- lm(utilization_rate ~ ., data = train)
lm_train_pred <- predict(lm_model, newdata = train)
lm_test_pred <- predict(lm_model, newdata = test)

lm_train_rmse <- get_rmse(train$utilization_rate, lm_train_pred)
lm_test_rmse <- get_rmse(test$utilization_rate, lm_test_pred)
lm_train_r2 <- get_r2(train$utilization_rate, lm_train_pred)
lm_test_r2 <- get_r2(test$utilization_rate, lm_test_pred)
...
rf_train_rmse <- get_rmse(train$utilization_rate, rf_train_pred)
rf_test_rmse <- get_rmse(test$utilization_rate, rf_test_pred)
rf_train_r2 <- get_r2(train$utilization_rate, rf_train_pred)
rf_test_r2 <- get_r2(test$utilization_rate, rf_test_pred)
...
X_mat <- model.matrix(utilization_rate ~ ., train)[, -1]
y_vec <- train$utilization_rate
X_test_mat <- model.matrix(utilization_rate ~ ., test)[, -1]
...
lasso_test_rmse <- get_rmse(test$utilization_rate, lasso_test_pred)
lasso_train_r2 <- get_r2(y_vec, lasso_train_pred)
lasso_test_r2 <- get_r2(test$utilization_rate, lasso_test_pred)
...
svr_train_rmse <- get_rmse(train$utilization_rate, svr_train_pred)
svr_test_rmse <- get_rmse(test$utilization_rate, svr_test_pred)
svr_train_r2 <- get_r2(train$utilization_rate, svr_train_pred)
svr_test_r2 <- get_r2(test$utilization_rate, svr_test_pred)
...
test_rmse <- get_rmse(test$utilization_rate, test_pred)
test_r2 <- get_r2(test$utilization_rate, test_pred)
...
p1 <- ggplot(plot_data, aes(x = actual, y = predicted)) +
  ...
  labs(
    ...
    x = "Actual Utilization Rate",
    y = "Predicted Utilization Rate"
  ) +
  theme_minimal() +
  theme(
